...
function runTests(arrayFrom) {
	test('from has proper length', function (t) {
		t.equal(arrayFrom.length, 1);
		t.end();
	});

	var arrayFromMethodExists = ES.IsCallable(Array.from);

	test('from is not enumerable', { skip: !arrayFromMethodExists }, function (t) {
	...
	test('works with a mapping function', function (t) {
		...
		t.test('accepts an object thisArg', function (st) {
			var context = {};
			arrayFrom(original, function (value, index) {
